    ...
    return (
        <div data-tour="categories" className="flex gap-4 overflow-x-auto px-6 pb-6 pt-2 scrollbar-hide">
            ...
            <button
                ...
                className={`flex-shrink-0 flex flex-col items-center p-4 rounded-[30px] transition-all duration-500 border-2 ${activeCategory === 'todos'
            ...
            >
                ...
                <span className={`font-black text-[11px] uppercase tracking-wider ${activeCategory === 'todos' ? 'text-[#FF4B82]' : 'text-gray-400'}`}>
                    Todos
                </span>
            </button>
            ...
            {categories.map(cat => (
                <button
                    ...
                    className={`flex-shrink-0 flex flex-col items-center p-4 rounded-[30px] transition-all duration-500 border-2 ${activeCategory === cat.id
                        ? 'bg-white border-[#FF4B82] shadow-xl shadow-pink-100 scale-105'
                        : 'bg-white border-transparent hover:border-pink-100 shadow-sm hover:shadow-md'
                        }`}
                >
                    ...
                    <span className={`font-black text-[11px] uppercase tracking-wider text-center leading-tight ${activeCategory === cat.id ? 'text-[#FF4B82]' : 'text-gray-400'}`}>
                        {cat.nome}
                    </span>
                </button>
            ))}
