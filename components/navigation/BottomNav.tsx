...
export default function BottomNav({ activeTab, onTabChange, cartCount, favoriteCount, isGuest }: BottomNavProps) {
    ...
    ].filter(item => !(isGuest && item.id === 'profile'));

    return (
        <div className="fixed bottom-6 left-0 w-full px-6 z-50 flex justify-center pointer-events-none">
            <nav className="bg-white/80 backdrop-blur-2xl border border-white/40 shadow-[0_20px_40px_rgba(0,0,0,0.1)] rounded-[32px] h-20 w-full max-w-[400px] flex items-center justify-around px-4 pointer-events-auto">
                {navItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className={`relative transition-all duration-300 ${isActive ? 'scale-110 -translate-y-1' : 'group-hover:scale-110'}`}>
                                {item.icon(isActive)}
                            ...
                            <span className={`text-[11px] font-black mt-1.5 transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-0 scale-75 h-0'}`}>
                                {item.label}
                            </span>
